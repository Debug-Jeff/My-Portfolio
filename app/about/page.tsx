...
const EnhancedAnimatedCode = () => {
  ...
  const getTextColor = (type) => {
    switch (type) {
      ...
      case "property": return "text-yellow-400 dark:text-yellow-300"
    ...
    <div className="font-mono text-sm leading-relaxed">
      ...
      {currentLine < codeLines.length && (
        <div className={`${getTextColor(codeLines[currentLine].type)} mb-1`}>
          <span className="text-muted-foreground mr-3">{String(currentLine + 1).padStart(2, '0')}</span>
          {displayedText}
          <motion.span
            animate={{ opacity: [1, 0] }}
...
export function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20 text-foreground relative overflow-hidden transition-colors duration-300">
      {/* Background Effects */}
      <FloatingParticles />
      <MatrixRain />
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <motion.div
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] opacity-5 blur-sm"
        ...
        >
          <div className="w-full h-full rounded-full bg-gradient-to-br from-primary/10 to-secondary/10 flex items-center justify-center overflow-hidden">
            <div className="w-full h-full bg-gradient-to-br from-muted/30 to-muted-foreground/30 rounded-full flex items-center justify-center">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(59,130,246,0.05),transparent_50%)] dark:bg-[radial-gradient(circle_at_50%_50%,rgba(59,130,246,0.1),transparent_50%)] pointer-events-none"></div>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16 min-h-[calc(100vh-200px)]">
          ...
          >
            ...
            <motion.div
              ...
              className="mb-6"
            >
              <span className="inline-block px-4 py-2 bg-gradient-to-r from-primary/10 to-secondary/10 rounded-full text-sm font-medium border border-primary/20 backdrop-blur-sm">
            ...
            >
              ...
              <Link href="/contact">
                <motion.button
                  className="px-8 py-4 border-2 border-primary text-primary hover:bg-primary/10 rounded-lg font-semibold transition-all duration-300 backdrop-blur-sm"
                  whileHover={{ scale: 1.05, y: -2 }}
                  whileTap={{ scale: 0.95 }}
                >
                  Contact Me
          ...
          >
            <div className="bg-card/80 backdrop-blur-md border border-border rounded-2xl shadow-2xl p-6 w-full max-w-lg relative overflow-hidden">
              {/* Terminal Header */}
